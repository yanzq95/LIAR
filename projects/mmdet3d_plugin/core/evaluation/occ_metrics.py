...
class Metric_mIoU():
    ...
    def count_miou(self):
        ...
        # 改动的地方：出现nan的时候替换为0
        mIoU_new = np.nan_to_num(mIoU, nan=0)
        print('\n')
        print('#######################          For night eval          #######################')
        print(f'===> mIoU(17) of {self.cnt} samples: ' + str(round(np.nanmean(mIoU_new[:self.num_classes-1]) * 100, 2)))
        print(f'===> mIoU(14) of {self.cnt} samples: ' + str(round((np.sum(mIoU_new[:self.num_classes-1])/14) * 100, 2)))
        print('#################################################################################')
        #################################################################################################################
