...
class LSS_forward(BaseModule):
    ...
    def __init__(
        ...
        super(LSS_forward, self).__init__()
    ...
    def create_frustum(self, depth_cfg, input_size, downsample):
        ...
        d = torch.arange(*depth_cfg, dtype=torch.float)\
            .view(-1, 1, 1).expand(-1, H_feat, W_feat)      # (D, fH, fW) self.D=44
        self.D = d.shape[0]
    ...
    def view_transform_core(self, input, depth, tran_feat):
        ...
        Args:
            input (list(torch.tensor)):
                imgs:  (B, N, C, fH, fW)        # N_views = 6 * (N_history + 1)
                sensor2egos: (B, N, 4, 4)
                ego2globals: (B, N, 4, 4)
                intrins:     (B, N, 3, 3)
                post_rots:   (B, N, 3, 3)
        ...
        if self.accelerate:
            ...
            # import numpy as np
            # np.savez(
            #     r"E:\PHD\my_project\ideas\template\FlashOCC\lss_learn\vis_file\bev_feat.npz",
    ...
    def forward(self, input):
        ...
        return bev_feat, tran_feat, depth
...
class LSS_forward_BEVDepth(LSS_forward):
    def __init__(self, loss_depth_weight=3.0, depthnet_cfg=dict(), **kwargs):
        super(LSS_forward_BEVDepth, self).__init__(**kwargs)
    ...
    def forward_bev_feat(self, input, stereo_metas):
        """
        Args:
            input (list(torch.tensor)):
                imgs:  (B, N_views, 3, H, W)        # N_views = 6 * (N_history + 1)
...
@NECKS.register_module()
class LSS_forward_BEVStereo(LSS_forward_BEVDepth):
    def __init__(self,
                 backward_projection=None,  # BEVFormer components
                 **kwargs):
        super(LSS_forward_BEVStereo, self).__init__(**kwargs)
    ...
    def forward(self, input, stereo_metas=None, illu_map=None):
        """
        Args:
            input (list(torch.tensor)):
                imgs:  (B, N_views, 3, H, W)        # N_views = 6 * (N_history + 1)
        ...
        Returns:
            ...
            depth: (B*N, D, fH, fW)
        """
        bev_forward_feat, context, depth = self.forward_bev_feat(input, stereo_metas)

        _, C, H, W = context.shape  # 和transformer中模块对齐
        ...
        bev_feat_refined, illu_bev_weight = self.backward_projection(
            mlvl_feats=[context.view(-1, 6, C, H, W)],
            pred_img_depth=depth.view(-1, 6, Cd, H, W),
            img_inputs=input,
            lss_bev=bev_forward_feat,
            illu_map=illu_map
        )

        illu_bev_weight = torch.clamp(illu_bev_weight, min=0.0, max=1.0)  # (B,1,200,200)
        bev_feat = bev_feat_refined * (illu_bev_weight + 0.50) + bev_forward_feat
