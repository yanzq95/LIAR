...
# temporal config
def construct_input(input_shape):
    rot = torch.eye(4).float().cuda().view(1, 1, 4, 4).expand(1,18,4,4)

    intrins = torch.eye(3).float().cuda().view(1,1, 3, 3).expand(1,18,3,3)
    input = dict(img_inputs=[
        torch.ones(()).new_empty((1, 18, *input_shape)).cuda(), rot,
        rot, intrins, intrins,
        torch.ones((1, 18, 3)).cuda(),
        torch.eye(3).float().cuda().view(1, 3, 3)],
        img_metas=[dict()])
    return input
